...
class DataProcess():
    ...
    def __init__(self, data_folder='Dream', file_name='AI_ICT_DKT.csv', min_inter_num=3):
    ...
    def process_csv(self):
        ...
        df = df.dropna(subset=['CLASSA'])
        df = df[~df['CLASSA'].isin(['noskill'])]
        print('After removing empty CLASSA, records number %d' % len(df))

        # # delete scaffolding problems 
        # 우리 데이터에는 해당 열이 없기 때문에 해당 코드를 삭제
    ...
    def pro_skill_graph(self):
        ...
        problems = df['QUIZCODE'].unique()
        pro_id_dict = dict(zip(problems, range(len(problems))))
        print('problem number %d' % len(problems))

        # pro_type = df['answer_type'].unique()
        pro_type = df['STUDY_TYPE'].unique()
        pro_type_dict = dict(zip(pro_type, range(len(pro_type))))
        print('STUDY_TYPE: ', pro_type_dict)

        pro_feat = []
        pro_skill_adj = []
        CLASSA_dict, skill_cnt = {}, 0
        for pro_id in range(len(problems)):            
            # tmp_df = df[df['problem_id']==problems[pro_id]]
            tmp_df = df[df['QUIZCODE']==problems[pro_id]]
            tmp_df_0 = tmp_df.iloc[0]

            # pro_feature: [ms_of_response, answer_type, mean_correct_num]
            # ms = tmp_df['ms_first_response'].abs().mean()
            # 시간 차이 계산 (밀리초 단위)
            df['time_diff_ms'] = (df['EDATE'] - df['SDATE']).dt.total_seconds() * 1000
            # 각 TESTCODE의 문제 풀이 시간 계산
            testcode_counts = df['TESTCODE'].value_counts()
            df['testcode_size'] = df['TESTCODE'].map(testcode_counts)
            df['avg_problem_time_ms'] = df['time_diff_ms'] / df['testcode_size']
            ms = tmp_df['avg_problem_time_ms']
            p = tmp_df['correct'].mean()
            pro_type_id = pro_type_dict[tmp_df_0['STUDY_TYPE']] 
            ...
            tmp_skills = [ele for ele in tmp_df_0['CLASSA'].split('_')]
            for s in tmp_skills:
                if s not in CLASSA_dict:
                    CLASSA_dict[s] = skill_cnt
                    skill_cnt += 1
                pro_skill_adj.append([pro_id, CLASSA_dict[s], 1])
        ...
        self.save_dict(CLASSA_dict, os.path.join(self.data_folder, 'CLASSA_dict.txt'))
    ...
    def generate_user_sequence(self, seq_file):
        ...
        df = pd.read_csv(os.path.join(self.data_folder, '%s_processed.csv'%self.data_folder), low_memory=False, encoding="ISO-8859-1")
        ui_df = df.groupby(['USERID'], as_index=True)   
        ...
        for ui in ui_df:
            ...
            tmp_problems = list(tmp_inter['QUIZCODE'])
            tmp_skills = list(tmp_inter['CLASSA'])
    ...
    def read_user_sequence(self, filename, max_len=200, min_len=3, shuffle_flag=True):
        with open(filename, 'r') as f:
            lines = f.readlines()
        with open(os.path.join(self.data_folder, 'CLASSA_dict.txt'), 'r') as f:
            CLASSA_dict = eval(f.read()) 
        with open(os.path.join(self.data_folder, 'pro_id_dict.txt'), 'r') as f:
            pro_id_dict = eval(f.read())
        

        y, skill, problem, real_len = [], [], [], []
        skill_num, pro_num = len(CLASSA_dict), len(pro_id_dict)
        ...
        while index < len(lines):
            ...
            tmp_skills = [CLASSA_dict[ele]+1 for ele in tmp_skills]     # for assist09
